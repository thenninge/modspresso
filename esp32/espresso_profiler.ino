...
void handleCommand(const char* command) {
  ...
  if (cmd == "start_profile") {
    ...
    String profileId = doc["profileId"];
...
void checkHardwareButtons() {
  ...
  if (button1State != lastButton1State) {
    if (currentTime - lastButton1Time > DEBOUNCE_DELAY) {
      if (button1State == LOW && !isRunning && defaultProfile1 != "") {
  ...
  if (button2State != lastButton2State) {
    if (currentTime - lastButton2Time > DEBOUNCE_DELAY) {
      if (button2State == LOW && !isRunning && defaultProfile2 != "") {
...
bool storeProfile(uint8_t id, JsonObject profileData) {
  ...
  for (int i = 0; i < profile.segmentCount; i++) {
    ...
    profile.segments[i].startPressure = segment["startPressure"] * 10; // Convert to 0-120
    profile.segments[i].endPressure = segment["endPressure"] * 10;
